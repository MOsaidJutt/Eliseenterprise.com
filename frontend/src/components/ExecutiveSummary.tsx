...
const RISK = {
  high:   { bar: "bg-red-500",     badge: "bg-red-500/15 text-red-400 border-red-500/30",     border: "border-red-500/20",    bg: "bg-red-500/5",    label: "High Risk",  dot: "bg-red-500",    val: "text-red-400" },
  medium: { bar: "bg-amber-400",   badge: "bg-amber-400/15 text-amber-400 border-amber-400/30", border: "border-amber-500/20", bg: "bg-amber-500/5",  label: "At Risk",    dot: "bg-amber-400",  val: "text-amber-400" },
  low:    { bar: "bg-emerald-500", badge: "bg-emerald-500/15 text-emerald-400 border-emerald-500/30", border: "border-emerald-500/20", bg: "bg-emerald-500/5", label: "On Track", dot: "bg-emerald-500", val: "text-emerald-400" },
};
...
function Metric({ label, value, alert }: { label: string; value: string | number; alert?: boolean }) {
  return (
    <div className="bg-white/[0.03] rounded-xl p-3 border border-white/[0.06] text-center">
      ...
      <p className={`text-sm font-bold ${alert ? "text-red-400" : "text-slate-300"}`}>{value}</p>
    </div>
  );
}
...
export default function ExecutiveSummary({ kpis, observations, spi_by_contractor = [], milestones = [], float_erosion = [] }: Props) {
  ...
  return (
    <div className={`bg-[#0D1829] rounded-2xl border ${rs.border} overflow-hidden shadow-xl shadow-black/30`}>
      <div className={`h-[3px] w-full ${rs.bar}`} />
      <div className={`px-5 py-3 ${rs.bg} border-b ${rs.border}`}>
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <svg className="w-4 h-4 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
            <h3 className="text-sm font-bold text-slate-200">Executive Insight Summary</h3>
          </div>
          <span className={`shrink-0 text-[10px] font-bold px-2.5 py-1 rounded-lg border ${rs.badge}`}>{rs.label}</span>
        </div>
      </div>
      ...
      <div className="p-6">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
          ...
          <div className="lg:col-span-3 space-y-3">
            {bullets.length === 0 ? (
              <p className="text-sm text-slate-500">Programme data loaded. No critical issues auto-detected — review sections below.</p>
            ) : (
              bullets.map((b, i) => (
            ...
            <div className="bg-white/[0.03] rounded-xl p-4 border border-white/[0.06] mt-4">
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Recommended Action</p>
